...
DOC_DIR="D:\\000.Dev\\open-webui\\backend\\apps\\rag\\faiss\\data"
...
#
if os_is_mac:
    DOC_DIR="/Users/bombbie/CodeLearning/open-webui/backend/apps/rag/faiss/data"
...
faiss_client.load_collection_by_ext(DOC_DIR, "pdf")
faiss_client_openai = FAISS_CLIENT(VECTOR_DIR_OPENAI, LLM_MODEL,EMBEDDING_MODEL,OLLAMA_URL,type="openai")
faiss_client_openai.load_collection_by_ext(DOC_DIR, "pdf")
...
if not os_is_mac:
    ...
    faiss_client_hf.load_collection_by_ext(DOC_DIR, "pdf")
...
docs = faiss_client.search_parent_doc(query,collection_name)
for index, doc in enumerate(docs):
    print("------- after faiss_client.search_parent_doc -----------------")
    print(f"<<<{index}>>>{doc}")
print("\n\n\n\n")
...
docs_openai = faiss_client_openai.search_parent_doc(query,collection_name)
for index, doc in enumerate(docs_openai):
    print("------- after faiss_openai_client.search_parent_doc -----------------")
    print(f"<<<{index}>>>{doc}")
print("\n\n\n\n")
...
if not os_is_mac:
    ...
    #     print(f"<<<{index}>>>{doc}")
    docs_hf = faiss_client_hf.search_parent_doc(query,collection_name)
    for index, doc in enumerate(docs_hf):
        print("------- after faiss_huggingface_client.search_parent_doc -----------------")
        print(f"<<<{index}>>>{doc}")
...
print("\n\n\n\n")
docs = faiss_client.search_llm_parent_doc(query,collection_name)
print("------- after faiss_client.search_llm_parent_doc -----------------")
print(docs['answer'])

docs_openai = faiss_client_openai.search_llm_parent_doc(query,collection_name)
print("------- after faiss_openai_client.search_llm_parent_doc -----------------")
print(docs_openai['answer'])

if not os_is_mac:
    docs_hf = faiss_client_hf.search_llm_parent_doc(query,collection_name)
    print("------- after faiss_huggingface_client.search_llm_parent_doc -----------------")
    print(docs['answer'])